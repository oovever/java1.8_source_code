    ...
    implements Iterable<S>
{
    ...
    // The class or interface representing the service being loaded 存储META-INF中需要被SPI管理的接口名（文件名）
    private final Class<S> service;

    // The class loader used to locate, load, and instantiate providers 类加载器
    private final ClassLoader loader;

    // The access control context taken when the ServiceLoader is created
    private final AccessControlContext acc;

    // Cached providers, in instantiation order // 缓存实现类
     ...
     */
    public void reload() {
        providers.clear(); // 清除缓存中已经通过ServiceLoader加载过的实现
        lookupIterator = new LazyIterator(service, loader); // 初始化LazyIterator
    }

    private ServiceLoader(Class<S> svc, ClassLoader cl) {
        service = Objects.requireNonNull(svc, "Service interface cannot be null");
        loader = (cl == null) ? ClassLoader.getSystemClassLoader() : cl; //如果没有使用系统ClassLoader
    ...
    // on the line to the names list.解析每一行
    //
        ...
        implements Iterator<S>
    {
        ...
        String nextName = null; // 下一个实现接口名称
            ...
            if (configs == null) {
                try {
                    ...
                    if (loader == null)
                        ...
                        configs = loader.getResources(fullName); //存储文件URL地址
                } catch (IOException x) {
...
//                解析文件
        ...
        private S nextService() {
            ...
            try {
                S p = service.cast(c.newInstance()); //转化为对象对应的类或者实现的接口实例
                providers.put(cn, p);
                return p;
            } catch (Throwable x) {
                fail(service,
     ...
     */
    public static <S> ServiceLoader<S> load(Class<S> service) {
        ClassLoader cl = Thread.currentThread().getContextClassLoader();// 获取当前线程的classLoader
